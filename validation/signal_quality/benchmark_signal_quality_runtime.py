#!/usr/bin/env python3
"""Runtime benchmark for controlled signal-quality validation."""
...
spec = importlib.util.spec_from_file_location("noiseq", ROOT / "run_signal_quality_benchmark.py")
...
def replace_runtime_section(report_path: Path, summary) -> None:
    ...
    new = (
        ...
        "This benchmark uses the same 2520 non-overlapping 10 s units for all four boundaries and computes one quality decision per unit. "
        "Under these equivalent boundary semantics, complete-path runtime is greater than the corresponding quality-assessment-only runtime for both methods.\n\n"
    )
...
def update_text_files(summary) -> None:
    ...
    methods = ROOT / "manuscript_methods_text.md"
    ...
    results = ROOT / "manuscript_results_text.md"
...
def main() -> None:
    ...
    summary.to_csv(ROOT / "signal_quality_runtime_summary.csv", index=False)
    replace_runtime_section(ROOT / "signal_quality_validation_report.md", summary)
    ...
    pd.DataFrame(checks).to_csv(ROOT / "runtime_boundary_consistency_check.csv", index=False)
